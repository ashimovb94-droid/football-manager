...
def simulate_round(league, round_num):
    ...
    for match in matches:
        ...
        result = simulate_match(
            ...
            home_mentality=home_bot.mentality if home_bot else 'balanced',
            away_mentality=away_bot.mentality if away_bot else 'balanced',
        )
...
def _update_standings(match, season_id, db):
    ...
    if not home_s or not away_s: return
    home_s.played += 1; away_s.played += 1
    home_s.gf += match.home_score; home_s.ga += match.away_score
    away_s.gf += match.away_score; away_s.ga += match.home_score
...
def _update_bot_stats(home_bot, away_bot, result, db):
    if not home_bot or not away_bot: return
    ...
    if home_bot.losses_streak >= 3:
        home_bot.formation = random.choice(['4-4-2','4-3-3','4-2-3-1','3-5-2','5-3-2'])
        home_bot.style = random.choice(['attacking','defensive','balanced','pressing'])
        home_bot.losses_streak = 0

    if away_bot.losses_streak >= 3:
        away_bot.formation = random.choice(['4-4-2','4-3-3','4-2-3-1','3-5-2','5-3-2'])
        away_bot.style = random.choice(['attacking','defensive','balanced','pressing'])
